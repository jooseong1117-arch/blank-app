...
guards_db = {
    ...
    "전재환": {"cost": 8000, "power": 70, "dice": 20},
    "이유빈": {"cost": 8500, "power": 50, "dice": 15},
    "한부현": {"cost": 9000, "power": 30, "dice": 15},
    "강준서": {"cost": 9500, "power": 55, "dice": 25},
    "장준혁": {"cost": 9500, "power": 75, "dice": 25},
...
items_db = {
    ...
    "🍲식어버린김치찜": {"cost": 2000, "desc": "에세이를 빼앗겨도 2번 다시할 수 있습니다."},
...
# 0.돈무한모드
if "김유영" in selected_guards:
    synergy_messages.append("💡 **[🍞🍞🍞김유영 모드🍞🍞🍞]** 재미를 위한 돈 무한 모드입니다. (용돈 5배 증가)")
...
if "이준혁" in selected_guards and "권준혁" in selected_guards and "장준혁" in selected_guards:
    discount = 2500
    synergy_messages.append("💡 **[시너지 발견: 준혁준혁준혁]** 삼준혁이 모였습니다. (고용비용 2,500원 할인)")
...
# 10. 스팸 시너지
    if "이준혁" in selected_guards and "전재환" in selected_guards:
        synergy_messages.append(f"💡 **[시너지 발견: 감자재환]** 이준혁이 계속 🥔감자재환 상태입니다..ㅠㅠ (디버프 강화)")
...
# 13. 오따끄
if "류혜정" in selected_guards and "한부현" in selected_guards:
    synergy_messages.append("💡 **[시너지 발견: 잼얘공유]** 잼얘가 늘어납니다.")

# 14. 명예 일본인들
jpops = [g for g in selected_guards if g in ["정진성", "류신해", "류혜정", "오유찬", "윤석준", "최진영"]]
if len(jpops) >= 2:
    synergy_messages.append("💡 **[시너지 발견: 명예 일본인들]** JPOP으로 영역을 전개합니다. (아군 진형의 영구 방어선 +10)")
  ...
  # 시간 흐름 루프 시작
        for hour in range(1, target_hours + 1):
            ...
            if hour == 1:
                ...
                hour_log += "> 📱 :red[**[정치질]**] ❗이정인이 톡방에서 정치질을 하며 엄청난 살의를 내뿜습니다❗\n\n"
                ...
                if "최진영" in selected_guards:
                    hour_log += "> 💉 :blue[**[무력화]**] 최진영이 보안프로그램을 작동시켜, 사악한 해킹공격을 저지합니다!\n\n"
            elif hour == 24:
                ...
                if max_dice > 0:
                    ...
                    if roll == max_dice:
                        if guard == "이아람":
                            ...
                            ji_perm_debuff += 10 
                            hour_log += "> 🌀 :red[**[최면술]**] 오유찬이 이정인에게 최면을 걸어 졸리게 합니다! (이정인 영구 위력 -10)\n\n"
                            ...
                            if "이준혁" in selected_guards:
                                ...
                                hour_log += "> 🥔 :red[**[감자재환]**] 🥔🥔🥔🥔이준혁의 뇌절에 지친 전재환이 감자재환이 됩니다!🥔🥔🥔🥔\n\n"
                            else:
                                current_team_power += 30
                    ...
                    # 💥 대실패 발동 로직 - 주사위가 1이 떴을 때
                        ...
                        # 3. 다리부러짐
                        elif guard == "이아람" and leg_broke == 0:
                            current_team_power -= 50
                            leg_broke += 1
                            ...
                            hour_log += "> 🤸‍♀️:red[**[다시 도망치기]**] 얘들아 약속이 있어서 다음에 다시 올게~ 그렇게 또 재원이는 사라져버렸습니다....\n\n"
                        else:
            ...
            if current_team_power > effective_ji_attack and raw_gap >= 50:
                if current_team_power >= 9000:
                    ...
                    carried_shield = int(raw_gap * 0.6)
                else:
